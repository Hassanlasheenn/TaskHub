import { Directive, ElementRef, EventEmitter, HostListener, OnDestroy, Optional, Output, Self } from '@angular/core';
...
export class PasteImageDirective implements OnDestroy {
    ...
    private _uploadAndInsert(file: File): void {
        ...
        this._imageUploadService.uploadImage(file).pipe(takeUntil(this._destroy$)).subscribe({
            next: (url) => {
                this._insertAtCursor(`![image](${url})`);
                this.imageUploading.emit(false);
            },
            error: () => {
                this._toastService.error('Failed to upload image');
                this.imageUploading.emit(false);
            }
        });
    }
